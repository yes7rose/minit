...
pub async fn init_basic_items(tomls: &Vec<Map<String, Value>>, root_id: &str, root_group_id: &str) {
    for map in tomls {
        ...
        if let Some(items) = utils::get_init_items(map) {
            ...
            for mut item in items {
                // 检查是否重复
                let id = if let Ok(r) = item.get_str(ID_FIELD_ID.to_string()) {
                    r
                } else {
                ...
                if let Err(_r) =
                    entity::insert_entity(manage_id, &mut item, root_id, root_group_id).await
                {
